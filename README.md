...
> | 你手上是 | 怎么做 |
> |---|---|
> | **发布包**：从本仓库 [Releases](../../releases) 页下载的 zip（里面有 `.output` 目录） | 解压 → 双击 `启动阳光跑.bat`，**不用装任何依赖，几秒就能用** |
> | **源码包**：GitHub 上的 "Download ZIP"，或 `git clone` 下来的 | **没有 `.output`**，会走开发模式：需要先 `pnpm install`（约 400MB）再 `pnpm dev`，首次要等几分钟 —— 这条路是给要改代码的人用的 |
>
> 只想用工具的人，请用 **Releases 里的发布包**；否则你会看到一串构建/依赖相关的报错，那些不是给使用者看的。
...
| `pnpm dev` 报 `Invalid port in url` 或 `entry point "vuetify" cannot be marked as external` | 你用的是旧版源码包（`vite-plugin-vuetify` 2.0.1）。重新下载源码包并 `pnpm install`，或直接用 Releases 里的发布包 |
| 浏览器打开是 404 / 打不开 | 源码包走 dev 模式时只监听 IPv6 的 `[::1]`（已在配置里修）；用发布包不会有这问题 |
